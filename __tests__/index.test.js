...
const testDefinition = {
  ...
  fields: {
    ...
    abilityStrength: { options: OptionLiterals.NUMBER },
    abilityConstitution: { options: OptionLiterals.NUMBER },
    level: {
          ...
          fields: {
            ability: {
              options: {
                str: {
                  modifiers: {
                    abilityStrength: { category: '+', fn: s => s + 2 },
                  },
                },
                con: {},
              },
...
describe('creating a builder', () => {
  describe('constructor', () => {
    ...
    it('imposes modifications defined by the option choice', () => {
      const character = testCharBuilder()
        .choose('abilityStrength', 12)
        .choose('class', 'fighter')
        .choose('class.ability', 'str');
      expect(character.get('abilityStrength')).toEqual(14);
    });
  });
  describe('.fields()', () => {
  ...
  describe('.modifiers([fieldName])', () => {
    it('returns all functions modifying the choice', () => {
      const character = testCharBuilder()
        .choose('abilityStrength', 12)
        .choose('class', 'fighter')
        .choose('class.ability', 'str');
      expect(character.modifiers('abilityStrength')['+']).toHaveLength(1);
    });
    it('removes the mod if the choice is unmade', () => {
      const character = testCharBuilder()
        .choose('abilityStrength', 12)
        .choose('class', 'fighter')
        .choose('class.ability', 'str');
      expect(character.modifiers('abilityStrength')['+']).toHaveLength(1);
      const newCharacter = character.choose('class.ability', 'con');
      expect(newCharacter.modifiers('abilityStrength')).toBeUndefined();
    });
  });
});
